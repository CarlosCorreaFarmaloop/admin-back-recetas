export const validateStatusChange = (currentStatus: string, toStatus: string) => {
  const statusChangeMap: Record<string, string[]> = {
    ...
    ENTREGADO: ['LISTO_PARA_RETIRO', 'EN_DELIVERY', 'ASIGNAR_A_DELIVERY'],
    EN_OBSERVACION: ['VALIDANDO_RECETA', 'OBSERVACIONES_RECETAS', 'PREPARANDO', 'RECETA_VALIDADA', 'CANCELADO'],
    CANCELADO: ['VALIDANDO_RECETA', 'OBSERVACIONES_RECETAS', 'EN_OBSERVACION', 'RECETA_VALIDADA'],
    CREADO: [],
  };
